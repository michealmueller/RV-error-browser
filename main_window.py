...
class UploadWorker(QObject):
    ...
    def upload_with_progress(self):
            ...
            for attempt in range(self._max_retries):
                try:
                    ...
                    blob_client.upload_blob(
                        data,
                        overwrite=True,
                        raw_response_hook=lambda r: progress_hook(
                            r.context.get("upload_stream_current", 0)
                        ),
        ...
        try:
            ...
            platform = (
                "android"
                if self.blob_name.startswith("android-builds/")
                else "ios"
                if self.blob_name.startswith("ios-builds/")
                else "unknown"
            )
            ...
            try:
                ...
                if "AuthorizationPermissionMismatch" in str(tag_exc):
                    logger.error(
                        f"Failed to set blob tags for {self.blob_name}: {tag_exc}"
                    )
                    from quantumops.logging_utils import append_terminal_line

                    append_terminal_line(
                        None,
                        f"Failed to set blob tags: Permission denied. Please ensure your Azure identity has 'Storage Blob Data Contributor' or higher RBAC role.",
                        "error",
                        True,
                    )
                else:
            ...
            try:
                ...
                logger.error(
                    f"Failed to set blob metadata for {self.blob_name}: {meta_exc}"
                )

            logger.info(f"Set blob tags and metadata for {self.blob_name}: {tags}")
        except Exception as meta_exc:
            logger.error(
                f"Failed to set blob tags/metadata for {self.blob_name}: {meta_exc}"
            )
...
class LogStreamWorker(QThread):
    ...
    def __init__(
        self, azure_webapp: AzureWebApp, resource_group: str, web_app_name: str
    ):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            self.azure_webapp.stream_logs(
                self.resource_group, self.web_app_name, callback=log_callback
            )
        except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    def setup_database_tab(self, tab_widget):
        ...
        try:
            ...
            self.connection_combo.currentIndexChanged.connect(
                self.handle_connection_selected
            )
    ...
    def append_system_log(self, message: str, level: str = "info") -> None:
        ...
        try:
            ...
            formatted_message = (
                f'<span style="color: {color}">[{timestamp}]</span> {message}'
            )
    ...
    def handle_fetch_azure_logs(self) -> None:
        ...
        if not resource_group:
            self.append_terminal_line(
                f"Could not determine resource group for {selected_webapp}",
                msg_type="error",
            )
            return
    ...
    def _start_log_streaming(self, webapp_name: str, resource_group: str) -> None:
        ...
        try:
            ...
            self.append_terminal_line(
                f"Started streaming logs for {webapp_name}", msg_type="info"
            )

        except Exception as e:
            logger.error(f"Error starting log stream: {str(e)}")
            self.append_terminal_line(
                f"Error starting log stream: {str(e)}", msg_type="error"
            )
            self._hide_loading()
            self.fetch_logs_btn.setEnabled(True)
            self.streaming_azure_logs = False
    ...
    def handle_connect(self):
        ...
        password = self.connections[self.connection_combo.currentIndex() - 1][
            "password"
        ]
        try:
            self.append_terminal_line(
                f"Attempting to connect to database...", msg_type="system"
            )
            self.conn = database.connect_to_database(host, port, dbname, user, password)
            self.append_terminal_line(
                f"Successfully connected to database: {dbname}", msg_type="success"
            )
            ...
            self.append_terminal_line(
                f"Error connecting to database: {str(e)}", msg_type="error"
            )
            self.query_btn.setEnabled(False)
            self.disconnect_btn.setEnabled(False)
    ...
    def handle_disconnect(self):
        if self.conn:
            self.append_terminal_line(
                f"Attempting to disconnect from database...", msg_type="system"
            )
            database.disconnect_database(self.conn)
            self.conn = None
            self.append_terminal_line(
                "Successfully disconnected from database", msg_type="success"
            )
            self.query_btn.setEnabled(False)
            self.disconnect_btn.setEnabled(False)
    ...
    def handle_query(self):
        ...
        try:
            self.append_terminal_line(
                f"Executing query on table: {table_name}", msg_type="system"
            )
            ...
            self.append_terminal_line(
                f"Error querying table: {str(e)}", msg_type="error"
            )
    ...
    def handle_refresh_builds(self, platform):
        ...
        table = (
            self.android_builds_table
            if platform == "android"
            else self.ios_builds_table
        )
        try:
            self.append_terminal_line(
                f"Fetching {platform} builds...", msg_type="system"
            )
            ...
            for i, build in enumerate(builds_list):
                ...
                table.setItem(
                    i, 4, QTableWidgetItem(str(build.get("app_version", "N/A")))
                )
                ...
                fingerprint = (
                    fingerprint_full[:7]
                    if len(fingerprint_full) >= 7
                    else fingerprint_full
                )
                table.setItem(i, 7, QTableWidgetItem(fingerprint))
                table.setItem(
                    i, 8, QTableWidgetItem(str(build.get("build_number", "N/A")))
                )
            ...
            self.append_terminal_line(
                f"Fetched {len(builds_list)} {platform} builds.", msg_type="success"
            )
            builds.enable_refresh_button(self, platform)
        except Exception as e:
            self.append_terminal_line(
                f"Error fetching {platform} builds: {e}", msg_type="error"
            )
    ...
    def handle_push_to_azure(self, build, platform, progress_bar=None, row=None):
        ...
        if not build_url:
            self.append_terminal_line(
                "No build URL available for upload.", msg_type="error"
            )
            return
        local_path = None
        temp_dir = None
        try:
            self.append_terminal_line(
                f"Downloading build {build_id}...", msg_type="system"
            )
            ...
            if not self.validate_filename(blob_name, build, platform):
                self.append_terminal_line(
                    f"Filename validation failed: {blob_name}", msg_type="error"
                )
                ...
                if progress_bar and row is not None:
                    ...
                    push_btn.clicked.connect(
                        lambda: self.handle_push_to_azure(
                            build, platform, progress_bar, row
                        )
                    )
            ...
            if missing:
                self.append_terminal_line(
                    f"Missing Azure environment variables: {', '.join(missing)}",
                    msg_type="error",
                )
                logging.warning(
                    f"[UPLOAD] Missing Azure environment variables: {', '.join(missing)}"
                )
                if local_path and os.path.exists(local_path):
                    os.remove(local_path)
                if temp_dir and os.path.exists(temp_dir):
            ...
            self.append_terminal_line(
                f"Uploading {blob_name} to Azure...", msg_type="system"
            )
            ...
            def on_finished(url, success, error_msg):
                ...
                if success:
                    self.append_terminal_line(
                        f"Upload successful! URL: {url}", msg_type="success"
                    )
                    ...
                    try:
                        if self.conn:
                            ...
                            # Use pipeline run number for DB update if available
                            database.update_download_url(
                                self.conn, table_name, build_id, url
                            )
                            self.append_terminal_line(
                                f"Updated download_url in {table_name} for build {build_id}",
                                msg_type="success",
                            )
                    except Exception as db_exc:
                        self.append_terminal_line(
                            f"Failed to update download_url in DB: {db_exc}",
                            msg_type="error",
                        )
                else:
                    self.append_terminal_line(
                        f"Upload failed: {error_msg}", msg_type="error"
                    )
                if progress_bar and row is not None:
    ...
    def build_filename(self, build, platform):
        ...
        if not build_id:
            raise ValueError(
                "appBuildVersion is required in build dict for filename pattern"
            )
    ...
    def validate_filename(self, filename, build, platform):
        ...
        if not build_id:
            raise ValueError(
                "appBuildVersion is required in build dict for filename validation"
            )
        fingerprint = str(build.get("fingerprint", "fp"))[:7]
        ext = ".apk" if platform == "android" else ".ipa"
    ...
    def handle_add_connection(self):
        self.append_terminal_line(
            "Connection dialog not available - module missing.", msg_type="warning"
        )
    ...
    def handle_delete_connection(self):
        ...
        if idx < 0 or idx >= len(self.connections):
            QMessageBox.warning(
                self, "Delete Connection", "Please select a connection to delete."
            )
            return
        ...
        if reply == QMessageBox.Yes:
            ...
            self.update_connection_combo()
            self.append_terminal_line(
                f"Deleted connection '{conn_name}'.", msg_type="success"
            )
    ...
    def toggle_autoresize(self):
        ...
        self.append_terminal_line(
            f"Auto-resize window set to {enabled}", msg_type="system"
        )
    ...
    def set_branding_theme(self, brand):
        ...
        self.update_all_widget_styles()
        self.append_terminal_line(
            f"Branding theme set to {brand} (module missing)", msg_type="warning"
        )
    ...
    def update_sas_token(self):
        ...
        new_sas, ok = QInputDialog.getText(
            self, "Update SAS Token", "Enter new SAS token URL:", text=current or ""
        )
        if ok and new_sas:
            ...
            self.append_terminal_line(
                "SAS token update cancelled or empty.", msg_type="warning"
            )
    ...
    def update_log_styles(self):
        ...
        self.log_window.setStyleSheet(
            f"color: #fff; background: #222; border: 1px solid {colors['primary']};"
        )
    ...
    def update_all_widget_styles(self):
        ...
        for btn in [
            ...
            self.edit_conn_btn,
            self.del_conn_btn,
            self.android_refresh_btn,
            self.ios_refresh_btn,
            self.fetch_logs_btn,
        ]:
            if btn:
                btn.setStyleSheet(
    ...
    def show_sp_info_dialog(self):
        ...
        keys = [
            ...
            (
                "AZURE_STORAGE_ACCOUNT",
                os.environ.get("AZURE_STORAGE_ACCOUNT", "Not set"),
            ),
            (
                "AZURE_STORAGE_CONTAINER",
                os.environ.get("AZURE_STORAGE_CONTAINER", "Not set"),
            ),
        ]
