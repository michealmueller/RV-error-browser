...
class HealthCheckModel(QObject):
    ...
    def __init__(self, webapps: list):
        ...
        if webapps:
            for app in webapps:
                if (
                    hasattr(app, "name")
                    and hasattr(app, "health_check_url")
                    and app.health_check_url
                ):
    ...
    def _load_endpoints(self) -> Dict[str, str]:
        ...
        try:
            if self.config_file.exists():
                ...
                logger.info(
                    "Health endpoints config file not found, using and saving default endpoints."
                )
                default_endpoints = {
    ...
    def _handle_check_complete(self, webapp: str, is_healthy: bool) -> None:
        ...
        logger.info(
            f"Health check for {webapp}: {'Healthy' if is_healthy else 'Unhealthy'}"
        )
