...
import re
import qdarktheme
from quantumops.views.delegates import DetailsDelegate, format_details
from quantumops.views.dialogs import ConnectionDialog
...
class MainWindow(QMainWindow):
    ...
    def _create_menu_bar(self):
        ...
        try:
            ...
            menubar.setVisible(True)  # Ensure menu bar is visible
            menubar.setNativeMenuBar(False)  # Disable native menu bar for Linux compatibility
            ...
            about_action = QAction("About QuantumOps", self)
    ...
    def _clear_all_logs(self):
        ...
        try:
            # Clear log browser if it exists
            if hasattr(self, "log_browser"):
                self.log_browser.clear()
            # Clear any other log displays
    ...
    def _show_about(self):
        """Show about dialog."""
        try:
            QMessageBox.about(self, "About QuantumOps", 
                            "QuantumOps\nDatabase Error Browser with Azure Integration\n\nA modern application for monitoring and managing database errors.")
        except Exception as e:
            logger.error(f"Error showing about dialog: {str(e)}")

    def log_message(self, message: str, level: str = "INFO"):
        """Log a message to both the UI and the logger (from app.py)"""
        ...
        # Log to any available log browser
        if hasattr(self, "log_browser"):
